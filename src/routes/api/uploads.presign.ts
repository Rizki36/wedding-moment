...
import { frameKey, submissionAudioKey, submissionPhotoKey } from '../../server/storage/keys'
      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        if (kind === 'submission-photo' || kind === 'submission-audio') {
          ...
          if (!eventId || !submissionId || !contentType) {
            return Response.json({ error: 'eventId, submissionId, and contentType are required' }, { status: 400 })
          }
          if (kind === 'submission-audio' && !ext) {
            return Response.json({ error: 'ext is required for submission-audio' }, { status: 400 })
          }

          const event = await getEvent(eventId)
          if (!event || event.status !== 'active') {
            return Response.json({ error: 'Event is not accepting submissions' }, { status: 403 })
          }

          const key = kind === 'submission-photo'
