...
import { createSubmission, listSubmissionsForEvent } from '../src/server/functions/submissions'
...
describe('listSubmissionsForEvent', () => {
  it('lists only submissions for the given event, newest first', async () => {
    const eventA = await createEvent({ ownerId: 'owner-a', brideName: 'A', groomName: 'B', eventDate: '2026-09-01' })
    const eventB = await createEvent({ ownerId: 'owner-b', brideName: 'C', groomName: 'D', eventDate: '2026-09-01' })
  ...
  it('returns an empty array for an event with no submissions', async () => {
    const event = await createEvent({ ownerId: 'owner-c', brideName: 'E', groomName: 'F', eventDate: '2026-09-01' })
