...
import { r2Client, R2_ENDPOINT, R2_BUCKET_NAME } from '../src/server/storage/r2-client'
import { submissionPhotoKey, submissionAudioKey } from '../src/server/storage/keys'
...
describe('buildSubmissionsZipResponse', () => {
  ...
  afterAll(async () => {
    if (photoKey) await r2Client.fetch(`${R2_ENDPOINT}/${R2_BUCKET_NAME}/${photoKey}`, { method: 'DELETE' })
    if (audioKey) await r2Client.fetch(`${R2_ENDPOINT}/${R2_BUCKET_NAME}/${audioKey}`, { method: 'DELETE' })
  })

  it('produces a ZIP containing every submission photo and audio file, streamed straight from R2', async () => {
    const event = await createEvent({ ownerId: 'owner-zip-test', brideName: 'A', groomName: 'B', eventDate: '2026-09-01' })
    ...
    expect(response!.headers.get('Content-Disposition')).toContain(`${event.slug}-moments.zip`)
