...
import { getEventBySlug, createEvent } from '../src/server/functions/events'
...
describe('guest landing loader data', () => {
  ...
  it('returns the event for a known slug', async () => {
    const created = await createEvent({ ownerId: 'test-owner', brideName: 'A', groomName: 'B', eventDate: '2026-09-01' })
    const found = await getEventBySlug(created.slug)
    expect(found?.id).toBe(created.id)
  })

  it('treats a purged event as unavailable', async () => {
    const created = await createEvent({ ownerId: 'test-owner', brideName: 'C', groomName: 'D', eventDate: '2020-01-01' })
